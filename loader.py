...
class ct_dataset(Dataset):
    def __init__(self, mode, load_mode, saved_path, test_patient, img, patch_n=None, patch_size=None, transform=None):
        assert mode in ['demo', 'test'], "mode is 'train' or 'test'"
        ...
        if mode == 'demo':
            input_ = [f for f in input_path if test_patient in f and '_{}_'.format(img) in f]
            target_ = [f for f in target_path if test_patient in f and '_{}_'.format(img) in f]
...
def get_loader(mode='test', load_mode=0,
               ...
               img =1, patch_n=None, patch_size=None,
               transform=None, batch_size=32, num_workers=6):
    dataset_ = ct_dataset(mode, load_mode, saved_path, test_patient, img, patch_n, patch_size, transform)
